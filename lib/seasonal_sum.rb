...
class SeasonalSummary < StatTracker
  attr_reader :id

  def initialize(id)
    # @id = id
    @seasonal_summary_helper = seasonal_summary_helper(id)
  end

  def seasonal_summary_helper(id)
    team_game_id = 0
    ...
    all_game_teams.each do |game_id, hash_pair|
      ...
      if hash_pair[0].team_id == id
        # do stuff for my team with hair_pair 0
        # do stuff for opponent team with hair_pair 1
      elsif hash_pair[1].team_id == id
        # do stuff for opponent team with hair_pair 0
        # do stuff for my team with hair_pair 1
      end

      # hash_pair.each do |key, game_team_obj|
      #   if id == game_team_obj.team_id
  ...
  def win_percentage(seasonid, pre_post_keys),
    @seasonal_summary_helper[seasonid][:home]
  end

  def total_goals(seasonid, us_or_them, pre_post_keys)
    @seasonal_summary_helper[seasonid][us_or_them][pre_post_keys].sum(goals)
  end

  def seasonal_summary
    summary_hash = Hash.new

    @seasonal_summary_helper.each do |seasonid, hash_pair|
      summary_hash[seasonid][pre_post_keys] ||= {
        win_percentage: win_percentage(seasonid, pre_post_keys),
        total_goals_scored: total_goals(seasonid, :home, pre_post_keys),
        total_goals_against: total_goals(seasonid, :away, pre_post_keys),
        average_goals_scored: avg_goals(seasonid, :home, pre_post_keys),
        average_goals_against: avg_goals(seasonid, :away, pre_post_keys)
      }
    end


    # seasonal_summary_helper(id).each do |seasonid, hash_pair|
    ...
    #     end
    #     opponent_gts = opponent_summary(id, game_team_obj_array)
  ...
  def opponent_summary(id, gt_array)
    ...
    oppo_game_teams.reject {|gt_obj| gt_obj.id == id}
  end
end
